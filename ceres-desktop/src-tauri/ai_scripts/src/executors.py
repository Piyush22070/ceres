...
from .exceptions import SecurityError, CommandExecutionError
...
class AppleScriptExecutor:
    ...
    def execute(self, script: str) -> Dict:
        ...
        try:
            # Validate script
            if not script.strip():
                return {"messages": [{"text": "⚠️ Empty AppleScript provided", "type": "bot"}]}
            
            if not self.response_cleaner.validate_applescript_syntax(script):
                return {"messages": [{"text": "⚠️ Invalid AppleScript syntax", "type": "bot"}]}
            ...
            return {"messages": [{"text": f"⚠️ AppleScript timed out ({Config.COMMAND_TIMEOUT}s limit)", "type": "bot"}]}
        except Exception as e:
            # logger.error(f"AppleScript execution error: {e}")
            return {"messages": [{"text": f"⚠️ AppleScript execution failed: {str(e)}", "type": "bot"}]}
    ...
    def _handle_applescript_error(self, error_msg: str) -> Dict:
        ...
        if "execution error" in error_msg.lower():
            return {"messages": [{"text": f"⚠️ AppleScript Error: {error_msg}", "type": "bot"}]}
        elif "application isn't running" in error_msg.lower():
            return {"messages": [{"text": "⚠️ Target application is not running", "type": "bot"}]}
        elif "can't get" in error_msg.lower():
            return {"messages": [{"text": "⚠️ AppleScript couldn't access the requested element", "type": "bot"}]}
        else:
            return {"messages": [{"text": f"⚠️ Script failed: {error_msg}", "type": "bot"}]}
    ...
    def execute(self, command: str) -> Dict:
        ...
        try:
            ...
            if result.returncode == 0:
                ...
                return {"messages": [{"text": f"⚠️ Command failed (exit {result.returncode}): {error_output}", "type": "bot"}]}
                
        except SecurityError as e:
            # logger.warning(f"Security violation blocked: {e}")
            return {"messages": [{"text": f"🛡️ Security: {str(e)}", "type": "bot"}]}
        except subprocess.TimeoutExpired:
            return {"messages": [{"text": f"⚠️ Command timed out ({Config.COMMAND_TIMEOUT}s limit)", "type": "bot"}]}
        except FileNotFoundError as e:
            return {"messages": [{"text": f"⚠️ Command not found: {str(e)}", "type": "bot"}]}
        except Exception as e:
