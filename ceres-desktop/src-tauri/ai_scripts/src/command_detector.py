...
class CommandDetector:
    """Determines whether a command should be executed as AppleScript or shell"""
    ...
    def detect_command_type(command: str, user_request: str) -> str:
        ...
        # Check command content for AppleScript indicators
        if any(indicator in command_lower for indicator in Config.APPLESCRIPT_INDICATORS):
            return 'applescript'
        
        # Check user request for AppleScript indicators
        ...
        # Default to shell for CLI-like operations
        return 'shell'